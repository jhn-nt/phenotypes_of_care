"""Experiment 2: Race-Stratified Model Evaluation

Trains two XGBoost models and evaluates each per race/ethnicity:
  - Model 1: Trained on all patients (all quartiles)
  - Model 2: Trained on Q4 (best-care) patients only

Both models are tested on 30% held-out data from EACH race group,
revealing whether the fairness gap differs across racial/ethnic groups.
...
OUT_DIR = os.path.join(SCRIPT_DIR, "results", "race")
...
def load_data():
    ...
    n_null = df["race"].isna().sum()
    if n_null > 0:
        print(f"  Dropping {n_null} rows with null race")
...
# ---------------------------------------------------------------------------
# Per-Race Held-Out Table
# ---------------------------------------------------------------------------

def race_holdout_table(df, model, model_name):
    """Evaluate a trained model on 30% held-out from each race group."""
    print(f"\n{'=' * 100}")
    print(f"{model_name} RESULTS BY RACE")
    print(f"{'=' * 100}")

    fmt_header = (f"{'Test Set':>45s} {'N':>6s} {'Deaths':>7s} {'Mortality Rate':>15s} "
    ...
    for race in RACE_ORDER:
        ...
        race_data = df[race_mask].copy()
        X_r = race_data[ALL_FEATURES]
        y_r = race_data[TARGET]

        if y_r.nunique() < 2 or y_r.sum() < MIN_POS_FOR_AUC:
            continue

        _, X_test, _, y_test = train_test_split(
            X_r, y_r, test_size=0.3, stratify=y_r, random_state=RANDOM_STATE,
        )
        label = f"{model_name} – Test {race} (30%)"
        ...
        if m:
            ...
            print(f"{label:>45s} {n:>6d} {deaths:>7d} {mort_rate:>14.1f}% "
    ...
    if all_y_true:
        ...
        if combined:
            ...
            comb_label = f"{model_name} – All Races Combined (30%)"
            print(f"{comb_label:>45s} {n:>6d} {deaths:>7d} {mort_rate:>14.1f}% "
...
def plot_auc_by_race(results_m1, results_m2):
    """Grouped bar chart: AUC by race for Model 1 vs Model 2."""
    races = [r for r in RACE_ORDER if r in results_m1 and r in results_m2]
    auc_m1 = [results_m1[r]["AUC"] for r in races]
    auc_m2 = [results_m2[r]["AUC"] for r in races]
    ci_lo_m1 = [results_m1[r]["AUC"] - results_m1[r]["CI_low"] for r in races]
    ci_hi_m1 = [results_m1[r]["CI_high"] - results_m1[r]["AUC"] for r in races]
    ci_lo_m2 = [results_m2[r]["AUC"] - results_m2[r]["CI_low"] for r in races]
    ci_hi_m2 = [results_m2[r]["CI_high"] - results_m2[r]["AUC"] for r in races]
    ...
    ax.set_ylabel("AUC")
    ax.set_title("AUC by Race — Model 1 vs Model 2")
    ax.set_xticks(x)
    ...
    out_path = os.path.join(OUT_DIR, "auc_by_race.png")
    fig.savefig(out_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"\nSaved figure: {out_path}")


def plot_fairness_gap_by_race(results_m1, results_m2):
    """Bar chart: AUC delta (Model 1 - Model 2) per race."""
    races = [r for r in RACE_ORDER if r in results_m1 and r in results_m2]
    gaps = [results_m1[r]["AUC"] - results_m2[r]["AUC"] for r in races]

    fig, ax = plt.subplots(figsize=(10, 5))
    bars = ax.bar(races, gaps, color="#E63946", width=0.5)
    ...
    ax.set_title("Fairness Gap by Race — Care-Phenotype Disparity")
    ax.set_ylim(0, max(gaps) * 1.2)
    ax.grid(axis="y", alpha=0.3)

    out_path = os.path.join(OUT_DIR, "fairness_gap_by_race.png")
    fig.savefig(out_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved figure: {out_path}")


def plot_ppv_by_race(results_m1, results_m2):
    """Grouped bar: PPV by race for Model 1 vs Model 2."""
    races = [r for r in RACE_ORDER if r in results_m1 and r in results_m2]
    ppv_m1 = [results_m1[r]["PPV"] for r in races]
    ppv_m2 = [results_m2[r]["PPV"] for r in races]

    x = np.arange(len(races))
    width = 0.35

    fig, ax = plt.subplots(figsize=(10, 5))
    ax.bar(x - width / 2, ppv_m1, width, label="Model 1 (All Quartiles)", color="#457B9D")
    ax.bar(x + width / 2, ppv_m2, width, label="Model 2 (Q4 Only)", color="#E63946")

    ax.set_xlabel("Race / Ethnicity")
    ax.set_ylabel("Positive Predictive Value (PPV)")
    ax.set_title("PPV by Race — Clinical Reliability of Mortality Predictions")
    ax.set_xticks(x)
    ax.set_xticklabels(races)
    ax.set_ylim(0, 1.1)
    ax.legend()
    ...
    out_path = os.path.join(OUT_DIR, "ppv_by_race.png")
    fig.savefig(out_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved figure: {out_path}")
...
def save_csvs(results_m1, results_m2):
    for model_num, results in [(1, results_m1), (2, results_m2)]:
        ...
        pd.DataFrame(rows).to_csv(
            os.path.join(OUT_DIR, f"race_holdout_model_{model_num}.csv"),
            index=False,
        )
...
def main():
    ...
    results_m1 = race_holdout_table(df, model_1, "Model 1")
    results_m2 = race_holdout_table(df, model_2, "Model 2")

    plot_auc_by_race(results_m1, results_m2)
    plot_fairness_gap_by_race(results_m1, results_m2)
    plot_ppv_by_race(results_m1, results_m2)
